...
2. **运行一次状态检查**（仅此一次，不要重复运行）：
   ...
   ```
   （`$SCRIPT` 为本 Skill 目录下的 `scripts/finance_data.py`，见「数据持久化」节的路径定位说明）
...
所有数据通过 `scripts/finance_data.py` 管理。

**脚本路径定位**：本 Skill 安装目录下的 `scripts/finance_data.py`。不同平台安装位置不同，AI 应根据本文件（SKILL.md）的实际路径推导脚本路径：

```
SKILL_DIR = SKILL.md 所在目录
SCRIPT    = SKILL_DIR/scripts/finance_data.py
DATA_DIR  = SKILL_DIR/data
OUTPUT_DIR = SKILL_DIR/output
```

常见安装位置（按优先级尝试）：
- CodeBuddy: `~/.codebuddy/skills/mindful-finance-coach/`
- Claude Code: `~/.claude/skills/mindful-finance-coach/`
- 项目级: 当前工作区的 `.codebuddy/skills/mindful-finance-coach/` 或 `.claude/skills/mindful-finance-coach/`

## 输出目录

所有面向用户的 HTML 报告和可视化输出，统一写入 `output/` 目录（即 `$SKILL_DIR/output/`）。该目录由脚本自动创建，无需手动操作。
